...
The intercept coefficient value indicates the number of lobsters that we can expect at a non-MPA site. Specifically, our model indicates that we should expect to count an average of about 9 lobsters per year at a non-MPA site. The intercept indicates the change in the number of lobsters that we expect to see when we apply our treatment, or in this case, when we search for lobsters in an MPA. In this case, we expect an MPA site to have four more lobsters than a non-MPA site.
...
The treatment estimate indicates a 1.24% increase in lobsters at MPA sites. It is slightly lower than the estimate produced by our poisson model.
...
d. Discuss the following causal inference assumptions in the context of the MPA treatment effect estimator. Evaluate if each of the assumption are reasonable: 
    ...
    SUTVA assumes that there is no spillover, and that our non-MPA groups are not experiencing any of the effects of the MPA zone. This is not reasonable for this situation, because the existence of MPA sites means that more fishermen will have to move to non-MPA sites. Meaning lobsters that may not have been caught in non-MPA sites may now have an increased chance of being caught. Since all of the lobsters are living in the same ocean together, we cannot make this assumption. More controls need to be established, which might not be possible for this specific experiment.
    
    2) Excludability assumption
    
    Excludability assumption assumes that an observed outcome is due solely to the treatment. This assumption is not reasonable for this experiment, because there are countless things that could also be causing the effect that we see. Theoretically (but ridiculously) the price of lobster, presence of pirates, or the regional release of a Disney animated movie about lobsters could all produce effects comparable to what we observed. We cannot assume that our observed effect on lobster count is solely due to the presence of MPAs, but if similar experiments with varying controls give similar results for many years, this study and others can serve as evidence to the efficacy of MPAs.
